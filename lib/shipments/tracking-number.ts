...
/**
 * Generates a human-readable tracking number in the format PH-YYYY-NNNNNN,
 * e.g. PH-2026-000001. The numeric portion is derived from the count of
...
export async function generateTrackingNumber(req: PayloadRequest): Promise<string> {
  ...
  return `PH-${year}-${String(sequence).padStart(6, "0")}`;
}