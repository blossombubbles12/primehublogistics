...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const title = post.seo?.metaTitle?.trim() || `${post.title} | PrimeHub Logistics`;
  const description =
  ...
  return {
    ...
    openGraph: {
      ...
      authors: [post.author?.name ?? "PrimeHub Logistics"].filter(Boolean),
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const structured = {
    ...
    author: { "@type": "Person", name: post.author?.name || "PrimeHub Logistics" },
    publisher: {
      "@type": "Organization",
      name: "PrimeHub Logistics",
